...
Low-level reference for how OnePersonCapital packages are built for npm.
...
The CLI package, `one-person-capital`, imports code from workspace packages such as:

- `@one-person-capital/server`
- `@one-person-capital/db`
- `@one-person-capital/shared`
...
- `npx one-person-capital@canary onboard` can install them explicitly
- `npx one-person-capital onboard` continues to resolve `latest`
