...
ClipHub is the public registry where people share, discover, and download OnePersonCapital company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to OnePersonCapital what a package registry is to a programming language. OnePersonCapital already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in OnePersonCapital — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own OnePersonCapital instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the OnePersonCapital spec. It contains:
...
**Install (fresh start):**
```
one-person-capital install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local OnePersonCapital instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within OnePersonCapital, export your company as a template, then publish:

```
one-person-capital export --template my-company
one-person-capital publish cliphub my-company
```
...
```
one-person-capital cliphub sync
```
...
ClipHub is a **separate service** from OnePersonCapital itself. OnePersonCapital is self-hosted; ClipHub is a hosted registry that OnePersonCapital instances talk to.
...
| **OnePersonCapital CLI** | `one-person-capital install`, `one-person-capital publish`, `one-person-capital cliphub sync` — built into OnePersonCapital |
| **OnePersonCapital UI** | "Browse ClipHub" panel in the OnePersonCapital web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with OnePersonCapital) |
| Backend | TypeScript + Hono (consistent with OnePersonCapital) |
| Database | PostgreSQL |
...
4. Run `one-person-capital install cliphub:acme/lean-saas-shop`
5. OnePersonCapital creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in OnePersonCapital until it works well
2. Export: `one-person-capital export --template my-agency`
3. Publish: `one-person-capital publish cliphub my-agency`
...
3. Install just that agent: `one-person-capital install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to OnePersonCapital

ClipHub is **not required** to use OnePersonCapital. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to OnePersonCapital what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `one-person-capital install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in OnePersonCapital web UI
- [ ] `one-person-capital cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with OnePersonCapital's own philosophy)
