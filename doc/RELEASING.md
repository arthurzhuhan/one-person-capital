# Releasing OnePersonCapital

Maintainer runbook for shipping a full OnePersonCapital release across npm, GitHub, and the website-facing changelog surface.
...
2. **npm** — `one-person-capital` and public workspace packages are published
...
ONE_PERSON_CAPITALAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

Users install canaries with:

```bash
npx one-person-capital@canary onboard
```
...
OnePersonCapital uses one release branch per target stable version:
...
./scripts/release-start.sh minor --worktree-dir ../one-person-capital-release-0.4.0
...
```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary ONE_PERSON_CAPITALAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable ONE_PERSON_CAPITALAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
ONE_PERSON_CAPITAL_PORT=3234 ./scripts/clean-onboard-ref.sh
./scripts/clean-onboard-ref.sh HEAD
```

Minimum checks:

- `npx one-person-capital@canary onboard` installs
...
- [ ] `npm view one-person-capital@latest version` matches the new stable version
