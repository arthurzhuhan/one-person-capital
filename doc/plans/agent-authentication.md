...
Agents need API keys to authenticate with OnePersonCapital. The current approach
...
**Trust model:** The adapter process runs on the same machine as the OnePersonCapital
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** OnePersonCapital generates a token and passes it directly to the agent
...
`ONE_PERSON_CAPITAL_API_URL`, `ONE_PERSON_CAPITAL_AGENT_ID`, `ONE_PERSON_CAPITAL_COMPANY_ID`. We need to
add a `ONE_PERSON_CAPITAL_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a OnePersonCapital CLI
...
```
one-person-capital auth login
# Opens browser -> user confirms -> token stored at ~/.one-person-capital/credentials
```
...
managed by the OnePersonCapital server itself.
...
1. A company admin (user or agent) generates an **invite URL** from OnePersonCapital.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The OnePersonCapital SKILL.md (or a link to it)
   - What information OnePersonCapital needs from the agent (e.g. webhook URL, adapter
...
5. OnePersonCapital stores the pending registration.
...
7. On approval, OnePersonCapital provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** OnePersonCapital issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.one-person-capital.ing/skills/one-person-capital/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How OnePersonCapital should send you heartbeats",
...
1. Generate a OnePersonCapital invite link for the company.
...
4. A OnePersonCapital company member approves the new agent.
5. OnePersonCapital begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `ONE_PERSON_CAPITAL_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `one-person-capital auth login` for developer-managed remote agents.                                      |
