...
`PAP-447` asks how OnePersonCapital should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same OnePersonCapital instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into OnePersonCapital:

- some operators use OnePersonCapital to manage OnePersonCapital and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as OnePersonCapital
- Claude and Codex expose different built-in affordances, so OnePersonCapital should not overfit to one tool

## Core Product Decision

OnePersonCapital should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by OnePersonCapital
...
- OnePersonCapital should not assume that every adapter can see or use a host filesystem path directly
...
If OnePersonCapital makes worktrees agent-first, it will blur:
...
- OnePersonCapital core resolves and records execution workspace state
...
That gives OnePersonCapital room to support:
...
- OnePersonCapital handles workspace runtime services under the hood using project/workspace policy
...
Once OnePersonCapital is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So OnePersonCapital should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that OnePersonCapital cannot infer
...
- when issue moves to done, should OnePersonCapital auto-commit?
- should it auto-push?
...
OnePersonCapital already has the right foundation for a project-first model.
...
For server-spawned local adapters, OnePersonCapital already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that OnePersonCapital should mirror directly

Implication:

- for `codex_local`, OnePersonCapital should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but OnePersonCapital should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, OnePersonCapital should separate:
...
OnePersonCapital should be able to express intentions such as:
...
These are not specific to OnePersonCapital. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".one-person-capital/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a OnePersonCapital-specific `devServer` flag.
...
OnePersonCapital should distinguish between:
...
OnePersonCapital should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### OnePersonCapital Core

OnePersonCapital core should:
...
OnePersonCapital core should not:
...
In addition to the local git helper, OnePersonCapital should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to OnePersonCapital
...
For runtime services specifically, OnePersonCapital should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as OnePersonCapital-only dev-server flags
...
3. Inject resolved execution workspace details into `context.one-person-capitalWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / OnePersonCapital instance can be reused through env/config injection when appropriate
...
- OnePersonCapital can decide whether to reuse or start a fresh service deterministically
...
3. Test against OnePersonCapital-on-OnePersonCapital workflows first.
...
7. Workspace runtime services are modeled generically, not as OnePersonCapital-specific dev-server toggles.
