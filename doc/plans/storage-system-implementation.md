...
Add a single storage subsystem for OnePersonCapital that supports:
...
- API serves attachment bytes through authenticated OnePersonCapital endpoints.
...
- Default local path is under instance root: `~/.one-person-capital/instances/<instanceId>/data/storage`.
...
- `one-person-capital onboard` writes a valid `storage` config block by default.
- `one-person-capital configure --section storage` can switch between local and s3 modes.
...
- `one-person-capital doctor` reports actionable storage status.
