# OnePersonCapital Module System

## Overview

OnePersonCapital's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends OnePersonCapital's API, UI, and data model | Yes |
...
modules/
  observability/
    one-person-capital.module.json     # manifest (required)
    src/
...
### Manifest (`one-person-capital.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@one-person-capital/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@one-person-capital/core";
...
```
1. Scan modules/ directory for one-person-capital.module.json manifests
...
```jsonc
// one-person-capital.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@one-person-capital/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@one-person-capital/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:one-person-capital/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.one-person-capital.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm one-person-capital store list                    # browse available modules and templates
pnpm one-person-capital store install <module-id>     # install a module
pnpm one-person-capital store import <template-id>    # import a company template
pnpm one-person-capital store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one OnePersonCapital instance | `server:started` |
...
Add to `@one-person-capital/server`:
...
4. **Module config** — `one-person-capital.config.json` with per-module config, env var interpolation
...
10. **`@one-person-capital/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
