...
- New config/env settings:
  - `ONE_PERSON_CAPITAL_AGENT_JWT_SECRET`
  - `ONE_PERSON_CAPITAL_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `ONE_PERSON_CAPITAL_AGENT_JWT_ISSUER` (default: `one-person-capital`)
  - `ONE_PERSON_CAPITAL_AGENT_JWT_AUDIENCE` (default: `one-person-capital-api`)
...
1. In:
   ...
   inject `ONE_PERSON_CAPITAL_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `ONE_PERSON_CAPITAL_API_KEY`, do not overwrite it.
- Continue injecting:
