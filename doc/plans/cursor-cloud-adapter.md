...
This document defines the V1 design for a OnePersonCapital adapter that integrates with
...
4. synthesized stdout events for OnePersonCapital UI/CLI
...
3. **Agent callback auth to OnePersonCapital** uses a bootstrap exchange flow (no long-lived OnePersonCapital key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from OnePersonCapital endpoints, not full SKILL.md prompt injection.
...
- `one-person-capitalPublicUrl` (optional override; else `ONE_PERSON_CAPITAL_PUBLIC_URL` env)
...
## OnePersonCapital Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `ONE_PERSON_CAPITAL_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.one-person-capitalPublicUrl`
2. `process.env.ONE_PERSON_CAPITAL_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived OnePersonCapital credentials in prompt text.

Flow:

1. Before launch/follow-up, OnePersonCapital mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `one-person-capitalPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. OnePersonCapital validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all OnePersonCapital API calls.
...
1. Prompt includes a compact instruction to fetch skills from OnePersonCapital.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/one-person-capital`
   - `GET /api/skills/one-person-capital-create-agent` when needed
...
- resolve `one-person-capitalPublicUrl`
...
- include webhook object when enabled:
  - `url: <one-person-capitalPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active OnePersonCapital run context.
...
5. `one-person-capitalPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- one-person-capital public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from OnePersonCapital skill endpoints |
| OnePersonCapital auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from OnePersonCapital context.
3. Add richer PR metadata surfacing in OnePersonCapital UI.
