# OnePersonCapital Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for OnePersonCapital's plugin and extension architecture.
...
OnePersonCapital plugin design is based on the following assumptions:

1. OnePersonCapital is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core OnePersonCapital business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by OnePersonCapital core.
...
2. Let plugins add major capabilities without editing OnePersonCapital core.
...
4. Depend on project-local plugin folders such as `.one-person-capital/plugins`.
...
The single OnePersonCapital deployment an operator installs and controls.

### 5.2 Company

A first-class OnePersonCapital business object inside the instance.
...
A trusted in-process extension loaded directly by OnePersonCapital core.
...
An installable instance-wide extension package loaded through the OnePersonCapital plugin runtime.
...
OnePersonCapital has two extension classes.
...
OnePersonCapital already has a concrete workspace model:
...
Plugins live under the OnePersonCapital instance directory.

Suggested layout:

- `~/.one-person-capital/instances/default/plugins/package.json`
- `~/.one-person-capital/instances/default/plugins/node_modules/`
- `~/.one-person-capital/instances/default/plugins/.cache/`
- `~/.one-person-capital/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

OnePersonCapital should add CLI commands:

- `pnpm one-person-capital plugin list`
- `pnpm one-person-capital plugin install <package[@version]>`
- `pnpm one-person-capital plugin uninstall <plugin-id>`
- `pnpm one-person-capital plugin upgrade <plugin-id> [version]`
- `pnpm one-person-capital plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@one-person-capital/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@one-person-capital/plugin-linear",
  "version": "0.1.0",
  "one-person-capitalPlugin": {
...
```ts
export interface OnePersonCapitalPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumOnePersonCapitalVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that OnePersonCapital agents can use during runs.
...
- OnePersonCapital server starts one worker process per installed plugin
...
Receives one typed OnePersonCapital domain event.
...
export function definePlugin(definition: PluginDefinition): OnePersonCapitalPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: OnePersonCapitalPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@one-person-capital/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@one-person-capital/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@one-person-capital/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@one-person-capital/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@one-person-capital/plugin-sdk/ui`
...
The `@one-person-capital/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the OnePersonCapital secret provider system rather than a plain text input
...
1. Core OnePersonCapital data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing OnePersonCapital objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual OnePersonCapital product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if OnePersonCapital later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing OnePersonCapital secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm one-person-capital plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the OnePersonCapital server. This is a normative requirement, not optional.
...
### 27.1 `@one-person-capital/plugin-test-harness`
...
```ts
import { createTestHarness } from "@one-person-capital/plugin-test-harness";
...
For developing a plugin against a running OnePersonCapital instance:

- The operator installs the plugin from a local path: `pnpm one-person-capital plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-one-person-capital-plugin`) that scaffolds:

- `package.json` with correct `one-person-capitalPlugin` keys
...
- `@one-person-capital/plugin-workspace-files`
- `@one-person-capital/plugin-terminal`
- `@one-person-capital/plugin-git`
- `@one-person-capital/plugin-linear`
- `@one-person-capital/plugin-github-issues`
- `@one-person-capital/plugin-grafana`
- `@one-person-capital/plugin-runtime-processes`
- `@one-person-capital/plugin-stripe`
...
- `@one-person-capital/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@one-person-capital/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@one-person-capital/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@one-person-capital/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@one-person-capital/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@one-person-capital/plugin-sdk/ui`
...
- `@one-person-capital/plugin-test-harness`
- `create-one-person-capital-plugin` starter template
...
OnePersonCapital should not implement a generic in-process hook bag modeled directly after local coding tools.

OnePersonCapital should implement:
...
That is the complete target design for the OnePersonCapital plugin system.
