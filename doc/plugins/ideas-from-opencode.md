...
OnePersonCapital V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a OnePersonCapital-shaped design.

Assumption for this document: OnePersonCapital is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class OnePersonCapital objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into OnePersonCapital.

The main conclusion is:

- OnePersonCapital should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- OnePersonCapital should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- OnePersonCapital should use multiple extension classes instead of one generic plugin bag:
...
If OnePersonCapital does this well, the examples you listed become straightforward:
...
Relevant OnePersonCapital files reviewed for current extension seams:
...
It is too dangerous for OnePersonCapital core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for OnePersonCapital — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
OnePersonCapital should absolutely do this.
...
OnePersonCapital should copy this discipline.
...
OnePersonCapital should adopt this style for plugin actions, automations, and UI schemas.
...
OnePersonCapital can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for OnePersonCapital too.

## What OnePersonCapital Should Not Copy Directly
...
OnePersonCapital is a control plane for an operator-managed instance with company objects.
...
OnePersonCapital is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

OnePersonCapital should not auto-load arbitrary code from a workspace repo like `.one-person-capital/plugins` or project directories.
...
For OnePersonCapital, equivalent hooks into:
...
OnePersonCapital should not allow plugins to silently replace:
...
For OnePersonCapital it would be risky because it combines:
...
OnePersonCapital should require an explicit operator install step.

## Why OnePersonCapital Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | OnePersonCapital |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted OnePersonCapital instance |
...
That means OnePersonCapital should borrow the good ideas from `opencode` but use a stricter architecture.

## OnePersonCapital Already Has Useful Pre-Plugin Seams

OnePersonCapital has several extension-like seams already:
...
This is good news.
OnePersonCapital does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended OnePersonCapital Plugin Model
...
OnePersonCapital already has this pattern implicitly:
...
- `plugin` means integration code that talks to OnePersonCapital through a typed plugin protocol and capability model
...
- invoke explicit OnePersonCapital actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@one-person-capital/plugin-sdk/ui`) with design tokens and common components.
...
OnePersonCapital should treat plugin installation as a global instance-level action.

Examples:

- install `@one-person-capital/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over OnePersonCapital objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

OnePersonCapital already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for OnePersonCapital too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@one-person-capital/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@one-person-capital/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running OnePersonCapital instance.

A `create-one-person-capital-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how OnePersonCapital achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the OnePersonCapital server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. OnePersonCapital's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@one-person-capital/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@one-person-capital/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For OnePersonCapital

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@one-person-capital/plugin-sdk";

export default definePlugin({
  id: "@one-person-capital/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit OnePersonCapital entities
    });
    ...
    ctx.events.on("plugin.@one-person-capital/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@one-person-capital/plugin-sdk/ui";
...
Any OnePersonCapital plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific OnePersonCapital objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@one-person-capital/plugin-linear`)
...
- reuse core tables when the data is clearly part of OnePersonCapital itself
...
If a concept is becoming part of OnePersonCapital's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of OnePersonCapital itself
...
### 3. Scope plugin data to OnePersonCapital objects before adding custom schemas

A lot of plugin data naturally hangs off existing OnePersonCapital objects:
...
If OnePersonCapital eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@one-person-capital/plugin-workspace-files`
...
Package idea: `@one-person-capital/plugin-terminal`
...
Package idea: `@one-person-capital/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving OnePersonCapital.
- Board reviews the diff after a run without leaving OnePersonCapital.
...
- optional `events.emit` (e.g. `plugin.@one-person-capital/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@one-person-capital/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@one-person-capital/plugin-linear`

This plugin syncs OnePersonCapital work with Linear. It is useful for:

- importing backlog from Linear
- linking OnePersonCapital issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between OnePersonCapital and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into OnePersonCapital.
- OnePersonCapital issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@one-person-capital/plugin-github-issues`

This plugin syncs OnePersonCapital issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to OnePersonCapital issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with OnePersonCapital task governance
...
- Plugin settings:
  ...
  - whether new OnePersonCapital issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active OnePersonCapital issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into OnePersonCapital.
- GitHub webhooks update status/comment state in OnePersonCapital.
- A PR is linked back to the OnePersonCapital issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@one-person-capital/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@one-person-capital/plugin-grafana`

This plugin surfaces external metrics and dashboards inside OnePersonCapital. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating OnePersonCapital issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full OnePersonCapital plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the OnePersonCapital dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a OnePersonCapital issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of OnePersonCapital core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@one-person-capital/plugin-runtime-processes`
...
Package idea: `@one-person-capital/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into OnePersonCapital. It is useful for:
...
- Failed payment spikes or churn events can generate OnePersonCapital issues for follow-up.
...
- Stripe data should stay additive to OnePersonCapital core
...
- plugin UI bundle loading, host bridge, `@one-person-capital/plugin-sdk/ui`
...
- `@one-person-capital/plugin-test-harness` and `create-one-person-capital-plugin` starter template
...
OnePersonCapital should not implement "an OpenCode-style generic in-process hook system."
OnePersonCapital should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In OnePersonCapital
...
8. Ship `@one-person-capital/plugin-test-harness` and `create-one-person-capital-plugin` starter template.
