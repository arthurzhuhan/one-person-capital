...
This spec defines how OnePersonCapital actually runs agents while staying runtime-agnostic.
...
1. OnePersonCapital is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, OnePersonCapital manages lifecycle, and on exit OnePersonCapital parses JSON output and updates state.
...
```json
{
  "runLogStore": {
    ...
    "bucket": "one-person-capital-run-logs",
    "prefix": "runs/",
    "compress": true,
...
- `run.startedAt`
- `heartbeat.reason`
- `one-person-capital.skill` (shared OnePersonCapital skill text block)
...
2. Preferred transport is env vars (`ONE_PERSON_CAPITAL_*`) injected at runtime.
