...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.one-person-capital/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.one-person-capital/instances/default/data/storage` (`local_disk`)
...
- exactly one JSON entrypoint: `one-person-capital.manifest.json`
