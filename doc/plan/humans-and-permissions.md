...
- local mode that still feels instant (`npx one-person-capital run` and go)
...
2. CLI command `pnpm one-person-capital auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm one-person-capital onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm one-person-capital onboard`).
...
- expose key instance settings in API + CLI (`one-person-capital configure` / `one-person-capital onboard`)
...
7. `pnpm one-person-capital onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
