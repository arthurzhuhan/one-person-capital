...
import { parseProjectMentionHref } from "@one-person-capital/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`one_person_capital-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="one_person_capital-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("one_person_capital-mermaid-status", error && "one_person_capital-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="one_person_capital-mermaid-source">
...
export function MarkdownBody({ children, className }: MarkdownBodyProps) {
  ...
  return (
    <div
      className={cn(
        "one_person_capital-markdown prose prose-sm max-w-none break-words overflow-hidden prose-pre:whitespace-pre-wrap prose-pre:break-words prose-code:break-all",
            ...
            if (parsed) {
              ...
              return (
                <a
                  ...
                  className="one_person_capital-project-mention-chip"
