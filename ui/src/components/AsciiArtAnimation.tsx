...
const ONE_PERSON_CAPITAL_SPRITES = [
  [
    "  ╭────╮ ",
    " ╭╯╭──╮│ ",
    " │ │  ││ ",
...
type OnePersonCapitalSprite = (typeof ONE_PERSON_CAPITAL_SPRITES)[number];
...
function spriteSize(sprite: OnePersonCapitalSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = ONE_PERSON_CAPITAL_SPRITES[(baseCol + baseRow) % ONE_PERSON_CAPITAL_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = ONE_PERSON_CAPITAL_SPRITES[Math.floor(Math.random() * ONE_PERSON_CAPITAL_SPRITES.length)]!;
