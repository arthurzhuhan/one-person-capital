...
import type { AdapterEnvironmentTestResult } from "@one-person-capital/shared";
...
} from "@one-person-capital/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@one-person-capital/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@one-person-capital/adapter-gemini-local";
...
https://github.com/one_person_capital/companies/blob/main/default/ceo/AGENTS.md
                      ...
                      {[
                        ...
                        >
                          {opt.recommended && (
                            <Badge
                              variant="secondary"
                              className="absolute -top-1.5 right-1.5 text-[9px] px-1.5 py-0 leading-none bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20"
                            >
                              Recommended
                            </Badge>
                          )}
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <HintIcon text="OnePersonCapital works best if you create a new folder for your agents to keep their memories and stay organized. Create a new folder and put the path here." />
                        </div>
                        <div className="flex items-center gap-2 rounded-md border border-border px-1 py-1 focus-within:ring-1 focus-within:ring-ring transition-shadow bg-background">
                          <FolderOpen className="h-3.5 w-3.5 text-muted-foreground shrink-0 ml-1.5" />
                          <input
                            className="flex-1 bg-transparent outline-none text-sm font-mono placeholder:text-muted-foreground/50 py-1"
                  ...
                  {isLocalAdapter && (
                    <div className="space-y-2 rounded-md border border-border p-3">
                      ...
                      adapterEnvResult.status === "pass" ? (
                        <div className="flex items-center gap-2 rounded-md border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-xs text-emerald-600 dark:text-emerald-400 animate-in fade-in slide-in-from-bottom-1 duration-300">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {step === 4 && (
                    <Button size="sm" disabled={loading} onClick={handleLaunch}>
                      ...
                      {loading ? t("common.loading") : t("common.launch")}
                    </Button>
                  )}
                </div>
              </div>
...
function AdapterEnvironmentResult({
  ...
  const statusClass =
    result.status === "pass"
      ...
      : result.status === "warn"
      ? "text-amber-700 dark:text-amber-300 border-amber-500/30 bg-amber-500/5"
