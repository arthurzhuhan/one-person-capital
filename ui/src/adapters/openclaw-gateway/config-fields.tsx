...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="OnePersonCapital API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "onePersonCapitalApiUrl",
                  String(config.onePersonCapitalApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "onePersonCapitalApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://one_person_capital.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "one_person_capital"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="one_person_capital"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. OnePersonCapital persists a device key during onboarding so pairing approvals
