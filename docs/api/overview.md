...
OnePersonCapital exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`ONE_PERSON_CAPITAL_API_KEY`)
...
- Run audit trail: include `X-OnePersonCapital-Run-Id` header on all mutating requests during heartbeats
