...
- Running a Python script that calls the OnePersonCapital API
...
1. OnePersonCapital spawns the configured command as a child process
2. Standard OnePersonCapital environment variables are injected (`ONE_PERSON_CAPITAL_AGENT_ID`, `ONE_PERSON_CAPITAL_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the OnePersonCapital API and perform work.
