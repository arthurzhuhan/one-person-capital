...
Build a custom adapter to connect OnePersonCapital to any agent runtime.
...
2. Build environment with `buildOnePersonCapitalEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `one-person-capital run --watch` using `picocolors`.
...
Make OnePersonCapital skills discoverable to your agent runtime without writing to the agent's working directory:
