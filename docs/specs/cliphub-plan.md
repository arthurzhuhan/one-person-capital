# ClipHub: Marketplace for OnePersonCapital Team Configurations

> The "app store" for whole-company AI teams — pre-built OnePersonCapital configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for OnePersonCapital-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | OnePersonCapital company (agents, projects, governance) |
...
A complete OnePersonCapital company configuration:
...
Single-agent configurations designed to plug into a OnePersonCapital org:
...
Portable skill files that any OnePersonCapital agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with OnePersonCapital's skill loading system
...
interface Listing {
  ...
  one-person-capitalVersionMin: string;     // Minimum OnePersonCapital version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target OnePersonCapital company
...
| `POST` | `/api/listings/:id/install` | Install to OnePersonCapital company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target OnePersonCapital company (or create new)
  → ClipHub API calls OnePersonCapital API to:
  ...
  → Redirect to OnePersonCapital dashboard with new team running
```
...
### 5.3 Creator: Export from OnePersonCapital → Publish

```
Running OnePersonCapital company → "Export as Blueprint" (CLI or UI)
  → OnePersonCapital exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (OnePersonCapital brand blue/purple)
...
| Install flow | One-click deploy to OnePersonCapital company |
...
{
  "targetCompanyId": "uuid",         // Existing OnePersonCapital company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `one-person-capital-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as OnePersonCapital
- **Backend**: Node.js API (or extend OnePersonCapital server)
- **Database**: Postgres (can share OnePersonCapital's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with OnePersonCapital auth (or OAuth2)

### 9.2 Integration with OnePersonCapital

ClipHub can be:
- **Option A**: A separate app that calls OnePersonCapital's API to install blueprints
- **Option B**: A built-in section of the OnePersonCapital UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing OnePersonCapital UI and API.
...
- [ ] Install flow: blueprint → OnePersonCapital company
...
- [ ] "Export from OnePersonCapital" CLI command
