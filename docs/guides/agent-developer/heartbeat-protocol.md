...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and OnePersonCapital.
...
If `ONE_PERSON_CAPITAL_APPROVAL_ID` is set, handle the approval first:
...
- If `ONE_PERSON_CAPITAL_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-OnePersonCapital-Run-Id: {runId}
