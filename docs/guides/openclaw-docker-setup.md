...
How to get OpenClaw running in a Docker container for local development and testing the OnePersonCapital OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

OnePersonCapital includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-one-person-capital-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a OnePersonCapital host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-one-person-capital-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `ONE_PERSON_CAPITAL_HOST_PORT` (default `3100`)
- `ONE_PERSON_CAPITAL_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your OnePersonCapital deployment is `authenticated`, provide auth context:

```bash
ONE_PERSON_CAPITAL_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
ONE_PERSON_CAPITAL_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host OnePersonCapital server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed OnePersonCapital URL (typically `http://host.docker.internal:3100`).
- If OnePersonCapital rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm one-person-capital allowed-hostname host.docker.internal
```

Then restart OnePersonCapital and rerun the smoke script.
...
```bash
pnpm one-person-capital allowed-hostname <host>
```
