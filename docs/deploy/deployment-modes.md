...
OnePersonCapital supports two runtime modes with different security profiles.
...
```sh
pnpm one-person-capital allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, OnePersonCapital emits a one-time claim URL at startup:
...
```sh
ONE_PERSON_CAPITAL_DEPLOYMENT_MODE=authenticated pnpm one-person-capital run
```
