...
IMAGE_NAME="${IMAGE_NAME:-one_person_capital-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
ONE_PERSON_CAPITALAI_VERSION="${ONE_PERSON_CAPITALAI_VERSION:-latest}"
DATA_DIR="${DATA_DIR:-$REPO_ROOT/data/docker-onboard-smoke}"
HOST_UID="${HOST_UID:-$(id -u)}"
ONE_PERSON_CAPITAL_DEPLOYMENT_MODE="${ONE_PERSON_CAPITAL_DEPLOYMENT_MODE:-authenticated}"
ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE="${ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE:-private}"
ONE_PERSON_CAPITAL_PUBLIC_URL="${ONE_PERSON_CAPITAL_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@one_person_capital.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-one_person_capital-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e ONE_PERSON_CAPITAL_DEPLOYMENT_MODE="$ONE_PERSON_CAPITAL_DEPLOYMENT_MODE" \
      -e ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE="$ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE" \
      -e ONE_PERSON_CAPITAL_PUBLIC_URL="$ONE_PERSON_CAPITAL_PUBLIC_URL" \
      -e ONE_PERSON_CAPITAL_HOME="/one_person_capital" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "one_person_capitalai@${ONE_PERSON_CAPITALAI_VERSION}" auth bootstrap-ceo --data-dir "$ONE_PERSON_CAPITAL_HOME" --base-url "$ONE_PERSON_CAPITAL_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $ONE_PERSON_CAPITAL_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$ONE_PERSON_CAPITAL_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg ONE_PERSON_CAPITALAI_VERSION="$ONE_PERSON_CAPITALAI_VERSION" \
...
echo "    Public URL: $ONE_PERSON_CAPITAL_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $ONE_PERSON_CAPITAL_DEPLOYMENT_MODE/$ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE"
...
docker run -d --rm \
  ...
  -e ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE="$ONE_PERSON_CAPITAL_DEPLOYMENT_EXPOSURE" \
  -e ONE_PERSON_CAPITAL_PUBLIC_URL="$ONE_PERSON_CAPITAL_PUBLIC_URL" \
  -v "$DATA_DIR:/one_person_capital" \
  "$IMAGE_NAME" >/dev/null

docker logs -f "$CONTAINER_NAME" &
LOG_PID=$!

TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/one_person_capital-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$ONE_PERSON_CAPITAL_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $ONE_PERSON_CAPITAL_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$ONE_PERSON_CAPITAL_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
