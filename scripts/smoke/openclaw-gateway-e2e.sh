...
ONE_PERSON_CAPITAL_API_URL="${ONE_PERSON_CAPITAL_API_URL:-http://127.0.0.1:3100}"
API_BASE="${ONE_PERSON_CAPITAL_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-one_person_capital-smoke}"
...
ONE_PERSON_CAPITAL_API_URL_FOR_OPENCLAW="${ONE_PERSON_CAPITAL_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${ONE_PERSON_CAPITAL_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${ONE_PERSON_CAPITAL_AUTH_HEADER}" )
fi
if [[ -n "${ONE_PERSON_CAPITAL_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${ONE_PERSON_CAPITAL_COOKIE}" )
  ONE_PERSON_CAPITAL_BROWSER_ORIGIN="${ONE_PERSON_CAPITAL_BROWSER_ORIGIN:-${ONE_PERSON_CAPITAL_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${ONE_PERSON_CAPITAL_BROWSER_ORIGIN}" -H "Referer: ${ONE_PERSON_CAPITAL_BROWSER_ORIGIN}/" )
fi
...
api_request() {
  ...
  if [[ "$path" == http://* || "$path" == https://* ]]; then
    ...
    url="${ONE_PERSON_CAPITAL_API_URL%/}${path}"
  else
    url="${API_BASE}${path}"
  fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set ONE_PERSON_CAPITAL_COOKIE or ONE_PERSON_CAPITAL_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg one_person_capitalApiUrl "$ONE_PERSON_CAPITAL_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "one_person_capital",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        one_person_capitalApiUrl: $one_person_capitalApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/one_person_capital"
  local claimed_file="${workspace_dir}/one_person_capital-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/one_person_capital-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/one_person_capital-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/one_person_capital-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/one_person_capital"
    assert_status "200"
    {
      echo "ONE_PERSON_CAPITAL_API_URL=${ONE_PERSON_CAPITAL_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set ONE_PERSON_CAPITAL_API_KEY=${AGENT_API_KEY} in your run context before running One Person Capital heartbeat steps."
...
main() {
  ...
  wait_http_ready "${ONE_PERSON_CAPITAL_API_URL%/}/api/health" 15 || fail "One Person Capital API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "one_person_capital health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
