...
ONE_PERSON_CAPITAL_HOST_PORT="${ONE_PERSON_CAPITAL_HOST_PORT:-3100}"
ONE_PERSON_CAPITAL_HOST_FROM_CONTAINER="${ONE_PERSON_CAPITAL_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.one_person_capital-openclaw.override.yml"
...
detect_one_person_capital_base_url() {
  ...
  for candidate in "$ONE_PERSON_CAPITAL_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${ONE_PERSON_CAPITAL_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${ONE_PERSON_CAPITAL_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
one_person_capital_base_url="$(detect_one_person_capital_base_url || true)"
...
One Person Capital URL for OpenClaw container:
EOF
  if [[ -n "$one_person_capital_base_url" ]]; then
    cat <<EOF
  $one_person_capital_base_url
  (Use this base URL for invite/onboarding links from inside OpenClaw Docker.)
EOF
  else
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${ONE_PERSON_CAPITAL_HOST_PORT}
  (Do not use http://127.0.0.1:${ONE_PERSON_CAPITAL_HOST_PORT} inside the container.)
  If One Person Capital rejects the host, run on host machine:
    pnpm one_person_capitalai allowed-hostname host.docker.internal
  Then restart One Person Capital and re-run this script.
EOF
  fi
  cat <<EOF
