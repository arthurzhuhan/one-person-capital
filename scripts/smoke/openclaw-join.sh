...
COMPANY_ID="${COMPANY_ID:-${ONE_PERSON_CAPITAL_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-one_person_capital-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-one_person_capital-openclaw-smoke}"
...
Provide one of:
  ONE_PERSON_CAPITAL_AUTH_HEADER=\"Bearer <board-token>\"
  ONE_PERSON_CAPITAL_COOKIE=\"<board-session-cookie>\"
...
log "checking One Person Capital health"
...
assert_status "200"
if ! grep -q "One Person Capital OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.one_person_capital.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
