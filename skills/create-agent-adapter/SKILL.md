...
description: >
  Technical guide for creating a new OnePersonCapital agent adapter. Use when building
...
# Creating a OnePersonCapital Agent Adapter

An adapter bridges OnePersonCapital's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `one-person-capital run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@one-person-capital/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@one-person-capital/adapter-utils` (types) or `@one-person-capital/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell OnePersonCapital to forget the stored session
}
...
```json
{
  "name": "@one-person-capital/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@one-person-capital/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including OnePersonCapital agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@one-person-capital/adapter-utils/server-utils`)
2. **Build environment** — call `buildOnePersonCapitalEnv(agent)` then layer in `ONE_PERSON_CAPITAL_RUN_ID`, context vars (`ONE_PERSON_CAPITAL_TASK_ID`, `ONE_PERSON_CAPITAL_WAKE_REASON`, `ONE_PERSON_CAPITAL_WAKE_COMMENT_ID`, `ONE_PERSON_CAPITAL_APPROVAL_ID`, `ONE_PERSON_CAPITAL_APPROVAL_STATUS`, `ONE_PERSON_CAPITAL_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `ONE_PERSON_CAPITAL_AGENT_ID` | `agent.id` |
| `ONE_PERSON_CAPITAL_COMPANY_ID` | `agent.companyId` |
| `ONE_PERSON_CAPITAL_API_URL` | Server's own URL |
| `ONE_PERSON_CAPITAL_RUN_ID` | Current run id |
| `ONE_PERSON_CAPITAL_TASK_ID` | `context.taskId` or `context.issueId` |
| `ONE_PERSON_CAPITAL_WAKE_REASON` | `context.wakeReason` |
| `ONE_PERSON_CAPITAL_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `ONE_PERSON_CAPITAL_APPROVAL_ID` | `context.approvalId` |
| `ONE_PERSON_CAPITAL_APPROVAL_STATUS` | `context.approvalStatus` |
| `ONE_PERSON_CAPITAL_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `ONE_PERSON_CAPITAL_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `one-person-capital run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@one-person-capital/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@one-person-capital/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use OnePersonCapital API
...
import { parseMyAgentStdoutLine } from "@one-person-capital/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@one-person-capital/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@one-person-capital/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so OnePersonCapital wipes the stale session
...
Import from `@one-person-capital/adapter-utils/server-utils`:
...
| `buildOnePersonCapitalEnv(agent)` | Standard `ONE_PERSON_CAPITAL_*` env vars |
...
- Package name: `@one-person-capital/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your OnePersonCapital work."`
...
### OnePersonCapital Skills Injection

OnePersonCapital ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `one-person-capital` API skill and the `one-person-capital-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with OnePersonCapital internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("one-person-capital-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "one-person-capital-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(ONE_PERSON_CAPITAL_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(ONE_PERSON_CAPITAL_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks OnePersonCapital skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
- `ONE_PERSON_CAPITAL_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the OnePersonCapital API and GitHub should not have access to arbitrary hosts.
