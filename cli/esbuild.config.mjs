/**
 * esbuild configuration for building the one-person-capital CLI for npm.
 *
 * Bundles all workspace packages (@one-person-capital/*) into a single file.
...
// to npm and resolved at runtime (e.g. @one-person-capital/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@one-person-capital/")) {
      externals.add(name);
    }
  }
