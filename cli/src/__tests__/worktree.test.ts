...
function buildSourceConfig(): OnePersonCapitalConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("one_person_capital-pr-432")).toBe(
      path.resolve(os.homedir(), "one_person_capital-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("one_person_capital/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://one_person_capital.example", 3110)).toBe("https://one_person_capital.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/one_person_capital-feature",
      homeDir: "/tmp/one_person_capital-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/one_person_capital-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/one_person_capital-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );

    const env = buildWorktreeEnvEntries(paths);
    expect(env.ONE_PERSON_CAPITAL_HOME).toBe(path.resolve("/tmp/one_person_capital-worktrees"));
    expect(env.ONE_PERSON_CAPITAL_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.ONE_PERSON_CAPITAL_IN_WORKTREE).toBe("true");
    expect(formatShellExports(env)).toContain("export ONE_PERSON_CAPITAL_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "one_person_capital-worktree-secrets-"));
    const originalInlineMasterKey = process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY;
      delete process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "one_person_capital-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".one_person_capital-worktrees"),
      });

      const envPath = path.join(repoRoot, ".one_person_capital", ".env");
      expect(fs.readFileSync(envPath, "utf8")).toContain("ONE_PERSON_CAPITAL_AGENT_JWT_SECRET=worktree-shared-secret");
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET;
      } else {
        process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/one_person_capital",
        targetRepoRoot: "/Users/example/one_person_capital-pr-432",
        workspaceCwd: "/Users/example/one_person_capital",
      }),
    ).toBe("/Users/example/one_person_capital-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/one_person_capital",
        targetRepoRoot: "/Users/example/one_person_capital-pr-432",
        workspaceCwd: "/Users/example/one_person_capital/packages/db",
      }),
    ).toBe("/Users/example/one_person_capital-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "one_person_capital-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "one_person_capital-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "one_person_capital-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("one_person_capital-make-test", {
        seed: false,
        home: path.join(tempRoot, ".one_person_capital-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".one_person_capital", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".one_person_capital", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
