...
describe("OnePersonCapitalApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new OnePersonCapitalApiClient({
    ...
    expect(headers["x-one_person_capital-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new OnePersonCapitalApiClient({ apiBase: "http://localhost:3100" });
