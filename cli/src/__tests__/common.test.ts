...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "one_person_capital-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.ONE_PERSON_CAPITAL_API_KEY;
    delete process.env.ONE_PERSON_CAPITAL_COMPANY_ID;
  });
