...
export const DEFAULT_WORKTREE_HOME = "~/.one_person_capital-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".one_person_capital");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: OnePersonCapitalConfig | null;
  paths: WorktreeLocalPaths;
  serverPort: number;
  databasePort: number;
  now?: Date;
}): OnePersonCapitalConfig {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "one_person_capital",
...
export function buildWorktreeEnvEntries(paths: WorktreeLocalPaths): Record<string, string> {
  return {
    ONE_PERSON_CAPITAL_HOME: paths.homeDir,
    ONE_PERSON_CAPITAL_INSTANCE_ID: paths.instanceId,
    ONE_PERSON_CAPITAL_CONFIG: paths.configPath,
    ONE_PERSON_CAPITAL_CONTEXT: paths.contextPath,
    ONE_PERSON_CAPITAL_IN_WORKTREE: "true",
  };
}
