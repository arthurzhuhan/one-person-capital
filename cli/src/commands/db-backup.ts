...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@one-person-capital/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://one_person_capital:one_person_capital@127.0.0.1:${port}/one_person_capital`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" one_person_capital db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveOnePersonCapitalInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "one_person_capital";
