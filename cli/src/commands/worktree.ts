...
import { ensureAgentJwtSecret, loadOnePersonCapitalEnvFile, mergeOnePersonCapitalEnvEntries, readOnePersonCapitalEnvEntries, resolveOnePersonCapitalEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.ONE_PERSON_CAPITAL_CONFIG;
...
function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.one_person_capital"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: OnePersonCapitalConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://one_person_capital:one_person_capital@127.0.0.1:${port}/one_person_capital`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: OnePersonCapitalConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY_FILE) : null);
...
async function ensureEmbeddedPostgres(dataDir: string, preferredPort: number): Promise<EmbeddedPostgresHandle> {
  ...
  const instance = new EmbeddedPostgres({
    databaseDir: dataDir,
    user: "one_person_capital",
    password: "one_person_capital",
    port,
    persistent: true,
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: OnePersonCapitalConfig;
  targetConfig: OnePersonCapitalConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveOnePersonCapitalEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readOnePersonCapitalEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://one_person_capital:one_person_capital@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "one_person_capital");
    const targetConnectionString = `postgres://one_person_capital:one_person_capital@127.0.0.1:${targetHandle.port}/one_person_capital`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readOnePersonCapitalEnvEntries(resolveOnePersonCapitalEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET);
  mergeOnePersonCapitalEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths),
      ...(existingAgentJwtSecret ? { ONE_PERSON_CAPITAL_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadOnePersonCapitalEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run OnePersonCapital inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" one_person_capital worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printOnePersonCapitalCliBanner();
  p.intro(pc.bgCyan(pc.black(" one_person_capital worktree:make ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveOnePersonCapitalEnvFile(configPath);
  const envEntries = readOnePersonCapitalEnvEntries(envPath);
  const out = {
    ONE_PERSON_CAPITAL_CONFIG: configPath,
    ...(envEntries.ONE_PERSON_CAPITAL_HOME ? { ONE_PERSON_CAPITAL_HOME: envEntries.ONE_PERSON_CAPITAL_HOME } : {}),
    ...(envEntries.ONE_PERSON_CAPITAL_INSTANCE_ID ? { ONE_PERSON_CAPITAL_INSTANCE_ID: envEntries.ONE_PERSON_CAPITAL_INSTANCE_ID } : {}),
    ...(envEntries.ONE_PERSON_CAPITAL_CONTEXT ? { ONE_PERSON_CAPITAL_CONTEXT: envEntries.ONE_PERSON_CAPITAL_CONTEXT } : {}),
    ...envEntries,
  };
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local OnePersonCapital instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated OnePersonCapital instance inside it")
    ...
    .option("--from-data-dir <path>", "Source ONE_PERSON_CAPITAL_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local OnePersonCapital instance")
