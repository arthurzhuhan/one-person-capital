...
function defaultConfig(): OnePersonCapitalConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" one_person_capital configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `one_person_capital onboard` first.");
    p.outro("");
    return;
  }

  let config: OnePersonCapitalConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because ONE_PERSON_CAPITAL_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
