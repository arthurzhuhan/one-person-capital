...
import type { Agent } from "@one-person-capital/shared";
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export ONE_PERSON_CAPITAL_API_URL='${escaped(input.apiBase)}'`,
    `export ONE_PERSON_CAPITAL_COMPANY_ID='${escaped(input.companyId)}'`,
    `export ONE_PERSON_CAPITAL_AGENT_ID='${escaped(input.agentId)}'`,
    `export ONE_PERSON_CAPITAL_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local OnePersonCapital skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing OnePersonCapital skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            ...
            if (!skillsDir) {
              throw new Error(
                "Could not locate local OnePersonCapital skills directory. Expected ./skills in the repo checkout.",
              );
            }
