...
import { ApiRequestError, OnePersonCapitalApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to OnePersonCapital config file")
    .option("-d, --data-dir <path>", "OnePersonCapital data directory root (isolates state from ~/.one_person_capital)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the OnePersonCapital API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.ONE_PERSON_CAPITAL_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.ONE_PERSON_CAPITAL_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.ONE_PERSON_CAPITAL_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set ONE_PERSON_CAPITAL_COMPANY_ID, or set context profile companyId via `one_person_capital context set`.",
    );
  }

  const api = new OnePersonCapitalApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.ONE_PERSON_CAPITAL_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.ONE_PERSON_CAPITAL_SERVER_PORT || "");
