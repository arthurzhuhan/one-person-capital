...
export function configCheck(configPath?: string): CheckResult {
  ...
  if (!configExists(configPath)) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `one_person_capital onboard` to create one",
    };
  }
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Run `one_person_capital configure --section database` (or `one_person_capital onboard` to recreate)",
    };
  }
}
