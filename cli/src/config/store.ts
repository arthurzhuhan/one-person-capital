...
import { onePersonCapitalConfigSchema, type OnePersonCapitalConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".one_person_capital", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveOnePersonCapitalInstanceId());
}
...
export function readConfig(configPath?: string): OnePersonCapitalConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = onePersonCapitalConfigSchema.safeParse(migrated);
  if (!parsed.success) {
