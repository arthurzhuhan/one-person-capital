...
const JWT_SECRET_ENV_KEY = "ONE_PERSON_CAPITAL_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# OnePersonCapital environment variables",
    "# Generated by OnePersonCapital CLI commands",
...
export function resolveOnePersonCapitalEnvFile(configPath?: string): string {
...
export function loadOnePersonCapitalEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeOnePersonCapitalEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readOnePersonCapitalEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeOnePersonCapitalEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeOnePersonCapitalEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readOnePersonCapitalEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeOnePersonCapitalEnvEntries(next, filePath);
  return next;
}
