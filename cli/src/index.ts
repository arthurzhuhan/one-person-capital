...
const DATA_DIR_OPTION_HELP =
  "OnePersonCapital data directory root (isolates state from ~/.one_person_capital)";

program
  .name("one_person_capital")
  .description("OnePersonCapital CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadOnePersonCapitalEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start OnePersonCapital immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your OnePersonCapital setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "one_person_capital")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run OnePersonCapital")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the OnePersonCapital server API")
