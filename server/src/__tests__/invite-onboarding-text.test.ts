...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("OnePersonCapital OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested OnePersonCapital base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("onePersonCapitalApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.onePersonCapitalApiUrl");
    expect(text).toContain("~/.openclaw/workspace/one_person_capital-claimed-api-key.json");
    expect(text).toContain("ONE_PERSON_CAPITAL_API_KEY");
