...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "one_person_capital-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "one_person_capital@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "OnePersonCapital Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".one_person_capital", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$ONE_PERSON_CAPITAL_WORKSPACE_BRANCH\" > .one_person_capital-provision-branch",
        "printf '%s\\n' \"$ONE_PERSON_CAPITAL_WORKSPACE_BASE_CWD\" > .one_person_capital-provision-base",
        "printf '%s\\n' \"$ONE_PERSON_CAPITAL_WORKSPACE_CREATED\" > .one_person_capital-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".one_person_capital-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".one_person_capital-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".one_person_capital-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".one_person_capital-provision-created"), "utf8")).resolves.toBe("false\n");
  });
});

describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "one_person_capital-runtime-workspace-"));
