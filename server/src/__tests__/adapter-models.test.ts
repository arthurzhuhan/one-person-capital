...
import { models as codexFallbackModels } from "@one-person-capital/adapter-codex-local";
import { models as cursorFallbackModels } from "@one-person-capital/adapter-cursor-local";
import { resetOpenCodeModelsCacheForTests } from "@one-person-capital/adapter-opencode-local/server";
...
describe("adapter model listing", () => {
  beforeEach(() => {
    ...
    delete process.env.ONE_PERSON_CAPITAL_OPENCODE_COMMAND;
  ...
  it("returns no opencode models when opencode command is unavailable", async () => {
    process.env.ONE_PERSON_CAPITAL_OPENCODE_COMMAND = "__one_person_capital_missing_opencode_command__";
