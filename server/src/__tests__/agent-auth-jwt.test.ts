...
describe("agent local JWT", () => {
  const secretEnv = "ONE_PERSON_CAPITAL_AGENT_JWT_SECRET";
  const ttlEnv = "ONE_PERSON_CAPITAL_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "ONE_PERSON_CAPITAL_AGENT_JWT_ISSUER";
  const audienceEnv = "ONE_PERSON_CAPITAL_AGENT_JWT_AUDIENCE";
  ...
  it("creates and verifies a token", () => {
    ...
    expect(claims).toMatchObject({
      ...
      run_id: "run-1",
      iss: "one_person_capital",
      aud: "one_person_capital-api",
    });
  });
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "one_person_capital";
    process.env[audienceEnv] = "one_person_capital-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
