...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "one_person_capital", LOGNAME: "one_person_capital", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "one_person_capital" }),
      },
    );

    expect(tokens).toEqual(["one_person_capital", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "one_person_capital" }, {
        userInfo: () => ({ username: "one_person_capital" }),
      });

      expect(tokens).toEqual(["one_person_capital", "custom-token"]);
    } finally {
  ...
  it("reports matches without leaking which token was searched", () => {
    ...
    const exitCode = runForbiddenTokenCheck({
      repoRoot: "/repo",
      tokens: ["one_person_capital", "custom-token"],
      exec,
      log,
      error,
    });
