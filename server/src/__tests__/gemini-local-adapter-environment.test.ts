...
import { testEnvironment } from "@one-person-capital/adapter-gemini-local/server";
...
describe("gemini_local environment diagnostics", () => {
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `one_person_capital-gemini-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("passes model and yolo flags to the hello probe", async () => {
    const root = path.join(
      os.tmpdir(),
      `one_person_capital-gemini-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
