...
import { execute, testEnvironment } from "@one-person-capital/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@one-person-capital/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@one-person-capital/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              wakeReason: "issue_assigned",
              issueIds: ["issue-123"],
              onePersonCapitalWorkspace: {
                cwd: "/tmp/worktrees/pap-123",
                strategy: "git_worktree",
                branchName: "pap-123-test",
              },
              onePersonCapitalWorkspaces: [
                {
                  id: "workspace-1",
                  cwd: "/tmp/project",
                },
              ],
              onePersonCapitalRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("one_person_capital:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("ONE_PERSON_CAPITAL_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("ONE_PERSON_CAPITAL_TASK_ID=task-123");
