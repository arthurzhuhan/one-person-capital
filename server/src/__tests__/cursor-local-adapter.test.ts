...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@one-person-capital/adapter-cursor-local/server";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$ONE_PERSON_CAPITAL_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $ONE_PERSON_CAPITAL_API_KEY\"";
