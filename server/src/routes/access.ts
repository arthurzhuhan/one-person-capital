...
import type { Db } from "@one-person-capital/db";
import {
  agentApiKeys,
  authUsers,
  invites,
  joinRequests
} from "@one-person-capital/db";
import {
  acceptInviteSchema,
  ...
  PERMISSION_KEYS
} from "@one-person-capital/shared";
import type { DeploymentExposure, DeploymentMode } from "@one-person-capital/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "one_person_capital" && normalized !== "one_person_capital-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  onePersonCapitalApiUrl?: unknown;
  ...
  if (!nonEmptyTrimmedString(merged.onePersonCapitalApiUrl)) {
    const legacyOnePersonCapitalApiUrl = nonEmptyTrimmedString(input.onePersonCapitalApiUrl);
    if (legacyOnePersonCapitalApiUrl) merged.onePersonCapitalApiUrl = legacyOnePersonCapitalApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    onePersonCapitalApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.onePersonCapitalApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawOnePersonCapitalApiUrl =
    typeof defaults.onePersonCapitalApiUrl === "string"
      ? defaults.onePersonCapitalApiUrl.trim()
      : "";
  if (rawOnePersonCapitalApiUrl) {
    try {
      const parsedOnePersonCapitalApiUrl = new URL(rawOnePersonCapitalApiUrl);
      if (
        parsedOnePersonCapitalApiUrl.protocol !== "http:" &&
        parsedOnePersonCapitalApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_one_person_capital_api_url_protocol",
          level: "warn",
          message: `onePersonCapitalApiUrl must use http:// or https:// (got ${parsedOnePersonCapitalApiUrl.protocol}).`
        });
      } else {
        normalized.onePersonCapitalApiUrl = parsedOnePersonCapitalApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_one_person_capital_api_url_configured",
          level: "info",
          message: `onePersonCapitalApiUrl set to ${parsedOnePersonCapitalApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_one_person_capital_api_url_invalid",
        level: "warn",
        message: `Invalid onePersonCapitalApiUrl: ${rawOnePersonCapitalApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your OnePersonCapital host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "OnePersonCapital is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm one_person_capital allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/one_person_capital";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/one_person_capital-claimed-api-key.json and load ONE_PERSON_CAPITAL_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: onePersonCapitalApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the OnePersonCapital hostname is reachable and allowed via `pnpm one_person_capital allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this OnePersonCapital API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        name: "one_person_capital",
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/one_person_capital/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # OnePersonCapital OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          onePersonCapitalApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, OnePersonCapital generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "onePersonCapitalApiUrl": "https://one_person_capital-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in OnePersonCapital before key claim is allowed.
    ...
    - ~/.openclaw/workspace/one_person_capital-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/one_person_capital-claimed-api-key.json

    And set the ONE_PERSON_CAPITAL_API_KEY and ONE_PERSON_CAPITAL_API_URL in your environment variables as specified here:
    ...
    {
      env: {
        ONE_PERSON_CAPITAL_API_KEY: "...",
        ONE_PERSON_CAPITAL_API_URL: "...",
      },
    }

    Then set ONE_PERSON_CAPITAL_API_KEY and ONE_PERSON_CAPITAL_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install OnePersonCapital skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your ONE_PERSON_CAPITAL_API_URL to the top of your skill and note the path to your ONE_PERSON_CAPITAL_API_URL
    ...
    ${
      ...
      "Ensure OnePersonCapital is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested OnePersonCapital base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.onePersonCapitalApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm one_person_capital allowed-hostname <host>
      - then restart OnePersonCapital and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@one_person_capital.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "one_person_capital", path: "/api/skills/one_person_capital" },
        {
          name: "one_person_capital-create-agent",
          path: "/api/skills/one_person_capital-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              onePersonCapitalApiUrl: req.body.onePersonCapitalApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.onePersonCapitalApiUrl &&
          !persistedDefaults.onePersonCapitalApiUrl
        ) {
          missingPersistedFields.push("onePersonCapitalApiUrl");
        }
