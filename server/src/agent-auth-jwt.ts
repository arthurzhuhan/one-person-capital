...
function jwtConfig() {
  const secret = process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.ONE_PERSON_CAPITAL_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.ONE_PERSON_CAPITAL_AGENT_JWT_ISSUER ?? "one_person_capital",
    audience: process.env.ONE_PERSON_CAPITAL_AGENT_JWT_AUDIENCE ?? "one_person_capital-api",
  };
}
