...
import { resolveOnePersonCapitalConfigPath, resolveOnePersonCapitalEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET === "string" ? parsed.ONE_PERSON_CAPITAL_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm one_person_capital onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveOnePersonCapitalEnvPath();
