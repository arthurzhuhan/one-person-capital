...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@one-person-capital/db";
import type { DeploymentMode } from "@one-person-capital/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  onePersonCapitalUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).onePersonCapitalUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.onePersonCapitalUpgradeContext = context;
