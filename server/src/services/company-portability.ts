...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@one-person-capital/shared";
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "one_person_capital" },
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.ONE_PERSON_CAPITAL_WORKSPACE_CWD);
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "one_person_capital.manifest.json"].filter(Boolean).join("/");
