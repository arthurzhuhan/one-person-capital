...
import type { AdapterRuntimeServiceReport } from "@one-person-capital/adapter-utils";
import type { Db } from "@one-person-capital/db";
import { workspaceRuntimeServices } from "@one-person-capital/db";
...
function sanitizeBranchName(value: string): string {
  return value
    ...
    .replace(/-+/g, "-")
    .replace(/^[-/.]+|[-/.]+$/g, "")
    .slice(0, 120) || "one_person_capital-work";
}
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.ONE_PERSON_CAPITAL_WORKSPACE_CWD = input.worktreePath;
  env.ONE_PERSON_CAPITAL_WORKSPACE_PATH = input.worktreePath;
  env.ONE_PERSON_CAPITAL_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.ONE_PERSON_CAPITAL_WORKSPACE_BRANCH = input.branchName;
  env.ONE_PERSON_CAPITAL_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.ONE_PERSON_CAPITAL_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.ONE_PERSON_CAPITAL_WORKSPACE_SOURCE = input.base.source;
  env.ONE_PERSON_CAPITAL_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.ONE_PERSON_CAPITAL_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.ONE_PERSON_CAPITAL_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.ONE_PERSON_CAPITAL_PROJECT_ID = input.base.projectId ?? "";
  env.ONE_PERSON_CAPITAL_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.ONE_PERSON_CAPITAL_AGENT_ID = input.agent.id;
  env.ONE_PERSON_CAPITAL_AGENT_NAME = input.agent.name;
  env.ONE_PERSON_CAPITAL_COMPANY_ID = input.agent.companyId;
  env.ONE_PERSON_CAPITAL_ISSUE_ID = input.issue?.id ?? "";
  env.ONE_PERSON_CAPITAL_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.ONE_PERSON_CAPITAL_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".one_person_capital", "worktrees");
