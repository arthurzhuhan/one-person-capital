...
const DEFERRED_WAKE_CONTEXT_KEY = "_onePersonCapitalWakeContext";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__one_person_capital_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.onePersonCapitalWorkspace = {
      cwd: executionWorkspace.cwd,
    ...
    context.onePersonCapitalWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.onePersonCapitalRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.onePersonCapitalRuntimeServiceIntents;
    }
    ...
    try {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[one_person_capital] ${warning}\n`);
      }
      ...
      if (runtimeServices.length > 0) {
        context.onePersonCapitalRuntimeServices = runtimeServices;
        context.onePersonCapitalRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[one_person_capital] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected ONE_PERSON_CAPITAL_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.onePersonCapitalRuntimeServices = combinedRuntimeServices;
        context.onePersonCapitalRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[one_person_capital] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
