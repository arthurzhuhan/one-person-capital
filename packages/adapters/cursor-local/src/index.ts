...
Use when:
- You want OnePersonCapital to run Cursor Agent CLI locally as the agent runtime
...
- OnePersonCapital auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$one_person_capital" and related skills on local runs.
- OnePersonCapital auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
