...
## Scope
This plan is now **gateway-only**. OnePersonCapital supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses OnePersonCapital skill.
...
```json
{
  "one-person-capitalApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a OnePersonCapital task.
...
```bash
pnpm --filter @one-person-capital/server test:run -- openclaw-gateway
pnpm --filter @one-person-capital/server typecheck
pnpm --filter @one-person-capital/ui typecheck
pnpm --filter one-person-capital typecheck
```
