...
} from "@one-person-capital/adapter-utils";
import { asNumber, asString, buildOnePersonCapitalEnv, parseObject } from "@one-person-capital/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "one_person_capital";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "one_person_capital";
  if (input.strategy === "run") return `one_person_capital:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `one_person_capital:issue:${input.issueId}`;
  return fallback;
}
...
function resolveOnePersonCapitalApiUrlOverride(value: unknown): string | null {
...
function buildOnePersonCapitalEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const onePersonCapitalApiUrlOverride = resolveOnePersonCapitalApiUrlOverride(ctx.config.onePersonCapitalApiUrl);
  const onePersonCapitalEnv: Record<string, string> = {
    ...buildOnePersonCapitalEnv(ctx.agent),
    ONE_PERSON_CAPITAL_RUN_ID: ctx.runId,
  };

  if (onePersonCapitalApiUrlOverride) {
    onePersonCapitalEnv.ONE_PERSON_CAPITAL_API_URL = onePersonCapitalApiUrlOverride;
  }
  if (wakePayload.taskId) onePersonCapitalEnv.ONE_PERSON_CAPITAL_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) onePersonCapitalEnv.ONE_PERSON_CAPITAL_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) onePersonCapitalEnv.ONE_PERSON_CAPITAL_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) onePersonCapitalEnv.ONE_PERSON_CAPITAL_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) onePersonCapitalEnv.ONE_PERSON_CAPITAL_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    onePersonCapitalEnv.ONE_PERSON_CAPITAL_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return onePersonCapitalEnv;
}

function buildWakeText(payload: WakePayload, onePersonCapitalEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/one_person_capital-claimed-api-key.json";
  const orderedKeys = [
    "ONE_PERSON_CAPITAL_RUN_ID",
    "ONE_PERSON_CAPITAL_AGENT_ID",
    "ONE_PERSON_CAPITAL_COMPANY_ID",
    "ONE_PERSON_CAPITAL_API_URL",
    "ONE_PERSON_CAPITAL_TASK_ID",
    "ONE_PERSON_CAPITAL_WAKE_REASON",
    "ONE_PERSON_CAPITAL_WAKE_COMMENT_ID",
    "ONE_PERSON_CAPITAL_APPROVAL_ID",
    "ONE_PERSON_CAPITAL_APPROVAL_STATUS",
    "ONE_PERSON_CAPITAL_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = onePersonCapitalEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = onePersonCapitalEnv.ONE_PERSON_CAPITAL_API_URL ?? "<set ONE_PERSON_CAPITAL_API_URL>";

  const lines = [
    "OnePersonCapital wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `ONE_PERSON_CAPITAL_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load ONE_PERSON_CAPITAL_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $ONE_PERSON_CAPITAL_API_KEY on every API call.",
    "- Use X-OnePersonCapital-Run-Id: $ONE_PERSON_CAPITAL_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: ONE_PERSON_CAPITAL_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$ONE_PERSON_CAPITAL_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$ONE_PERSON_CAPITAL_COMPANY_ID/issues?assigneeAgentId=$ONE_PERSON_CAPITAL_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardOnePersonCapitalPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  onePersonCapitalEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateOnePersonCapital = parseObject(payloadTemplate.one_person_capital);
  const workspace = asRecord(ctx.context.onePersonCapitalWorkspace);
  const workspaces = Array.isArray(ctx.context.onePersonCapitalWorkspaces)
    ? ctx.context.onePersonCapitalWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.onePersonCapitalRuntimeServiceIntents)
    ? ctx.context.onePersonCapitalRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardOnePersonCapital: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: onePersonCapitalEnv.ONE_PERSON_CAPITAL_API_URL ?? null,
  };

  if (workspace) {
    standardOnePersonCapital.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardOnePersonCapital.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardOnePersonCapital.workspaceRuntime = {
  ...
  return {
    ...templateOnePersonCapital,
    ...standardOnePersonCapital,
  };
}
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "one_person_capital-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const onePersonCapitalEnv = buildOnePersonCapitalEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, onePersonCapitalEnv);
  ...
  const onePersonCapitalPayload = buildStandardOnePersonCapitalPayload(ctx, wakePayload, onePersonCapitalEnv, payloadTemplate);
