...
import { asString, parseObject } from "@one-person-capital/adapter-utils/server-utils";
        ...
        ws.send(
          JSON.stringify({
            ...
            params: {
              ...
              client: {
                id: "gateway-client",
                version: "one_person_capital-probe",
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the OnePersonCapital server host.",
        });
      }
    } catch (err) {
      checks.push({
