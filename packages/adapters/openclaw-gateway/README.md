...
This document describes how `@one-person-capital/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into OnePersonCapital logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (OnePersonCapital `runId`)
