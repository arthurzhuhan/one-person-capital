...
import type { AdapterModel } from "@one-person-capital/adapter-utils";
import { asString, runChildProcess } from "@one-person-capital/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.ONE_PERSON_CAPITAL_PI_COMMAND === "string" &&
    process.env.ONE_PERSON_CAPITAL_PI_COMMAND.trim().length > 0
      ? process.env.ONE_PERSON_CAPITAL_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["ONE_PERSON_CAPITAL_", "npm_", "NPM_"] as const;
