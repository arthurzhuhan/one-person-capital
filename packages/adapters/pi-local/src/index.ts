...
Use when:
- You want OnePersonCapital to run Pi (the AI coding agent) locally as the agent runtime
...
- OnePersonCapital requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/one_person_capitals/ and resumed with --session.
