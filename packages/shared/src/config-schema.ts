...
export const databaseBackupConfigSchema = z.object({
  ...
  dir: z.string().default("~/.one_person_capital/instances/default/data/backups"),
});

export const databaseConfigSchema = z.object({
  mode: z.enum(["embedded-postgres", "postgres"]).default("embedded-postgres"),
  connectionString: z.string().optional(),
  embeddedPostgresDataDir: z.string().default("~/.one_person_capital/instances/default/db"),
  embeddedPostgresPort: z.number().int().min(1).max(65535).default(54329),
  backup: databaseBackupConfigSchema.default({
    enabled: true,
    intervalMinutes: 60,
    retentionDays: 30,
    dir: "~/.one_person_capital/instances/default/data/backups",
  }),
});

export const loggingConfigSchema = z.object({
  mode: z.enum(["file", "cloud"]),
  logDir: z.string().default("~/.one_person_capital/instances/default/logs"),
});
...
export const storageLocalDiskConfigSchema = z.object({
  baseDir: z.string().default("~/.one_person_capital/instances/default/data/storage"),
});

export const storageS3ConfigSchema = z.object({
  bucket: z.string().min(1).default("one_person_capital"),
...
export const storageConfigSchema = z.object({
  ...
  localDisk: storageLocalDiskConfigSchema.default({
    baseDir: "~/.one_person_capital/instances/default/data/storage",
  }),
...
export const secretsLocalEncryptedConfigSchema = z.object({
  keyFilePath: z.string().default("~/.one_person_capital/instances/default/secrets/master.key"),
});
...
export const secretsConfigSchema = z.object({
  ...
  localEncrypted: secretsLocalEncryptedConfigSchema.default({
    keyFilePath: "~/.one_person_capital/instances/default/secrets/master.key",
  }),
});

export const onePersonCapitalConfigSchema = z
  .object({
...
export type OnePersonCapitalConfig = z.infer<typeof onePersonCapitalConfigSchema>;
