...
function resolveOnePersonCapitalInstanceId(): string {
  const raw = process.env.ONE_PERSON_CAPITAL_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveOnePersonCapitalHomeDir(), "instances", resolveOnePersonCapitalInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveOnePersonCapitalHomeDir(), "instances", resolveOnePersonCapitalInstanceId(), "data", "backups");
}
