...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "one_person_capital-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveOnePersonCapitalHomeDir(), "instances", resolveOnePersonCapitalInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".one_person_capital", CONFIG_BASENAME);
...
function resolveOnePersonCapitalConfigPath(): string {
  if (process.env.ONE_PERSON_CAPITAL_CONFIG?.trim()) {
    return path.resolve(process.env.ONE_PERSON_CAPITAL_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveOnePersonCapitalEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveOnePersonCapitalEnvPath(configPath);
